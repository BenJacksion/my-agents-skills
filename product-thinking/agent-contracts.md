...
当产品结果需要进入系统、应用或项目设计时，交接内容至少包括：

- 目标用户、使用情境、现有替代方案和关键行为证据；
- 已确认的用户/业务结果、成功标准、指标、基线和质量护栏；
- 机会、候选方案、关键假设及其验证状态；
- 当前优先级、做与不做的机会成本、资源/时间/合规约束；
- 与系统边界、领域、数据、运行或交付有关的明确约束和未知项。

下游可以把这些内容作为架构输入，但不得把产品候选方案直接当成系统边界，也不得把产品假设升级为架构事实。若候选方案会改变系统边界或整体复杂度，应交给 `system-architecture-product-design`；若进入企业应用内部结构，再交给 `enterprise-application-architecture`。
