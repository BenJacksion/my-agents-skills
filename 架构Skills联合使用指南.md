...
仓库另有独立的平级能力域 [`enterprise-application-architecture/`](enterprise-application-architecture/)，专门处理企业应用内部的 5 视图、领域/数据/开发/运行/物理架构，以及分布式、云和数据平台；[`ontology-modeling/`](ontology-modeling/) 专门处理能力问题、共享语义、身份/关系/规则和本体映射。两者都不属于本指南的两域联合流程，使用方式见各自目录的 `README.md`。

如果需要把产品思维、系统架构、企业应用架构、《架构之道：软件构建的设计方法》和本体建模全部放入软件生命周期，参阅[软件工程流程](software-engineering-process/README.md)。
